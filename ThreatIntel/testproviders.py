...
with open("data/titan.crt", "rb") as certfile:
    titancert = certfile.read()
with open("data/titan.key", "rb") as keyfile:
    titankey = keyfile.read()

providers = [
    PhishTankDataProvider(),
    DShieldDataProvider(),
    ShadowServerDataProvider(),
    TitanDataProvider(titancert, titankey),
